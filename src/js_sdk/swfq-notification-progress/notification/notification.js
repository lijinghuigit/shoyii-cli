		...
		function NotificationCustom() {
			...
			if (Notification) {
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {//android8.0及以上需设置通知渠道才能显示通知
					...
					var importance = NotificationManager.IMPORTANCE_HIGH;//重要性级别（或者 IMPORTANCE_HIGH,IMPORTANCE_LOW,IMPORTANCE_MIN静默，下载时手机状态栏不展示箭头）
				    ...
				    mChannel.enableVibration(false);//是否振动（这块设置完之后需要对app重新安装才能生效）
				...
				*/
				...
				// this.mNotificationBuild.setSound(null,null);
				this.mNotificationBuild.setDefaults(plus.android.importClass("android.app.Notification").DEFAULT_SOUND);
				// this.mNotificationBuild.setDefaults(plus.android.importClass("android.app.Notification").FLAG_ONLY_ALERT_ONCE);
